...
def test_index_sell_rule_map_selection():
    ...
    assert svc._get_sell_rule_name("NIKKEI225") == "nikkei225_overheat_guard_score80_gate"
...
def test_nikkei225_overheat_guard_boost_only_for_matching_shape():
    svc = _svc()
    closes = [100.0] * 70 + [105.0] * 40 + [110.0 + i for i in range(20)]
    ma20 = sum(closes[-20:]) / 20
    ma60 = sum(closes[-60:]) / 60
    ma200 = 100.0
    boost = svc._nikkei225_overheat_guard_boost(closes, ma20, ma60, ma200)
    assert boost in {20.0, 35.0}

    non_matching = [100.0] * 130
    ma20_flat = sum(non_matching[-20:]) / 20
    ma60_flat = sum(non_matching[-60:]) / 60
    assert svc._nikkei225_overheat_guard_boost(non_matching, ma20_flat, ma60_flat, 100.0) == 0.0
