...
INDEX_SELL_RULE_MAP = {
    ...
    "NIKKEI225": "nikkei225_overheat_guard_score80_gate",
}
...
class BacktestService:
    ...
    def _nikkei225_overheat_guard_boost(self, closes: List[float], ma20: float, ma60: float, ma200: float) -> float:
        if len(closes) < 120:
            return 0.0
        close = closes[-1]
        if not (close > ma20 and close > ma60 and close > ma200):
        ...
        if not base_ok:
            return 0.0
        boost = 20.0
        if deviation_from_ma20_pct >= 4.0 and deviation_from_ma60_pct >= 5.0:
            boost += 15.0
        return boost
    ...
    def run_backtest(
        ...
        for idx, (date_str, close) in enumerate(price_history):
            ...
            if idx >= max(score_ma - 1, 199):
                ...
                nikkei225_overheat_guard_active = False
                if sell_rule_name in {"ath_boost_8_score80_gate", "no_ath_penalty_score80_gate"}:
                    ...
                    if shares > 0 and boost > 0:
                        ...
                        if topix_overheat_guard_active:
                            sell_gate_open = True
                elif sell_rule_name == "nikkei225_overheat_guard_score80_gate":
                    boost = self._nikkei225_overheat_guard_boost(closes, ma20_series[-1], ma60_series[-1], ma200_series[-1])
                    if shares > 0 and boost > 0:
                        score = max(0.0, min(100.0, score + boost))
                        nikkei225_overheat_guard_active = score >= sell_threshold_safe
                        if nikkei225_overheat_guard_active:
                            sell_gate_open = True
                    ...
                    trades.append(
                        {
                            ...
                            "reason": (
                                ...
                                if topix_overheat_guard_active
                                else "nikkei225_overheat_guard_bypass_peakout_confirmation"
                                if nikkei225_overheat_guard_active
                                else "topix_overheat_guard_score80_gate"
                                if sell_rule_name == "topix_overheat_guard_score80_gate"
                                else "nikkei225_overheat_guard_score80_gate"
                                if sell_rule_name == "nikkei225_overheat_guard_score80_gate"
